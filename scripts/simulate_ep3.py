# -*- coding: utf-8 -*-
"""Simulatore di playtest per Ombre su Roccamora - EPISODIO 3 «Le voci del pozzo».

COPIA di simulate_ep2.py (a sua volta dal riferimento storico dell'Ep. 1)
cucita sull'Episodio 3 come da bibbia: cisterne (TILES_3 di gen_ep3), boss
L'ACCORDATORE (umano, debolezza il Campanello di Piero), nemico nuovo LA
VOCE CAVA (quando abbattuta URLA: NERVI Facile agli adiacenti o -1 azione
- simulata), mazzo Minaccia proprio, luoghi/chiavi del caso del Ladro di
Voci. Le formule di scaling (MINACCIA_FORMULE, CUSTODE_TENSIONE_EXTRA,
SALUTE_BONUS_PER_N, TICK_CANTO_OGNI/SOGLIA_CANTO) sono RIUSATE identiche -
config di produzione 20260717.
Specifiche Ep. 3 modellate: la Galleria delle Eco (T3, NERVI Difficile per
ogni eroe alla prima entrata; nessuna prova con LA CANNA MUTA - trovata se
L6 visitato; declassata a Media col Presagio di L7 letto), il Campanello
di Piero (dal barbiere con la Testimonianza L2, altrimenti Cercando in T5),
...
from gen_ep3 import TILES_3 as TILES, NEMICI_3, LUOGHI_3  # noqa: E402

NEMICI = NEMICI_COMUNI + NEMICI_3
HERO = {h['nome']: h for h in HEROES}
LUOGHI_BY_N = {l['n']: l for l in LUOGHI_3}

# Mazzo Minaccia dell'episodio (21 carte - «La campana nuova» della
# variante Bivio NON e' simulata, dichiarato nel docstring). Tuple
...
TOKEN_POOL_BASE = {'ADEPTO INCAPPUCCIATO': 4, 'LA VOCE CAVA': 3,
...
# Ep.3: il Canto batte ogni 6 round, non ogni 4 (dato: webapp/data/ep3.json
# canto_ogni). E' una spedizione lunga e la clessidra comune la strangolava —
# misurato a 4 eroi, stessi party e stessi seed: 16% -> 62% di vittorie.
TICK_CANTO_OGNI = 6
SOGLIA_CANTO = 3
...
# Stessa semantica dell'Ep. 2: DISTANZA_PORTA = "sull'uscita piu' vicina
# agli eroi", None = arrivano da un'altra tessera (distanza dinamica) -
# il calare dei secchi (T4), il ritiro delle canne (T5) e gli inseguitori
# dalla Scala (T1) sono tutti "da fuori" rispetto al gruppo.
DISTANZA_PORTA = 1
SPAWN_DISTANZA = {
    ...
    'IL TURNO DI SOTTO': DISTANZA_PORTA,
    'PASSI NELLA GALLERIA': None,
...
# Il boss dell'episodio (vedi gen_ep3.NEMICI_3 e DESIGN-EPISODIO-3.md):
...
LUOGHI_SIM = [
    # Ep. 3: chiavi garantite negli indizi core (gen_ep3.LUOGHI_3), tutte da
    # luoghi aperti (1, 2, 3, 4). `campanello=True` = la Testimonianza di L2
    # consegna il Campanello di Piero (debolezza del boss); `canna=True` =
    # la Canna Muta (L6, indizio core); `incrocio_d3=True` = uno dei
    # riscontri della Domanda 3 (registro L1, falde L5, conto L6,
    # sopralluogo L9: ne servono almeno 2).
    dict(n=1, nome='Il Lavatoio Grande', req=None, sblocca_parola='ACQUA MORTA',
...
def simula_indagine(party, log, esplora_a_fondo=False):
    log('=' * 78)
    log('INDAGINE - Episodio 3: "Le voci del pozzo"')
    log('=' * 78)
    ...
    parole = set()
    oggetti = set()
    canna = False        # L6: la Canna Muta (T3 senza prova)
    incroci_d3 = 0       # riscontri della Domanda 3 (>=2 = pozzo giusto)
    approf_letti = 0
    approf_falliti = 0
    ...
    def punteggio(l):
        ...
        missione = 0 if ((l.get('canna') and not canna)
        ...
        for tipo in l['approf']:
            ...
            if idoneo:
                ...
                if (l['n'], tipo) in CHI_ESPLICITO:
                    chi_confermato = True
                    log('    -> Questa carta conferma esplicitamente che il colpevole è Silvano (Domanda 2).')
                if (l['n'], tipo) == (2, 'Testimonianza'):
                    log('    -> Silvano affida al gruppo IL CAMPANELLO DI PIERO (la debolezza del boss).')
            else:
    ...
    while ore > 0:
        ...
        candidato_missione = ((l.get('canna') and not canna)
        ...
        if l.get('canna'):
            canna = True
            log('    -> Trovata: LA CANNA MUTA (la Galleria delle Eco non vi sentirà).')
        if l.get('incrocio_d3'):
            incroci_d3 += 1
            log(f'    -> Riscontro sulla Domanda 3 ({incroci_d3}: registro/falde/conto/sopralluogo).')
        if ok:
    ...
    campanello = (2, 'Testimonianza') in approf_dettaglio
    d3_ok = incroci_d3 >= 2
    log(f'Campanello di Piero: {"sì" if campanello else "no (resta da cercare in T5)"}; '
        f'Canna Muta: {"sì" if canna else "no"}; Domanda 3 (pozzo giusto, {incroci_d3} riscontri): '
        f'{"esatta" if d3_ok else "SBAGLIATA — scenderete dal pozzo sbagliato"}')
    log(f'Chi è il Ladro di Voci confermato esplicitamente (Domanda 2): '
...
def simula_spedizione(party, indagine, log, run_seed, formula_minaccia='standard',
    ...
    log('=' * 78)
    log('SPEDIZIONE - Le Cisterne del Borgo (sotto il Lavatoio Grande)')
    log('=' * 78)
    log(f'Party: {", ".join(party)}')
    log(f'Bonus da Indagine: {indagine["tier"]}; Campanello di Piero: '
        f'{"sì" if indagine["campanello"] else "no"}; Canna Muta: '
    ...
    # Domanda 3 sbagliata: si scende dal pozzo sbagliato - 1 Voce Cava
    # appare in T1 alla rivelazione (Soluzione).
    sbarco_rumoroso = not indagine.get('d3_ok')
    if pool_extra:
    ...
    if sbarco_rumoroso:
        if pool['LA VOCE CAVA'] > 0:
            ...
            enemies.append(dict(nome='LA VOCE CAVA', fer=g_fer, fer_max=g_fer,
    ...
    def fase_eroi(luogo_label):
        nonlocal chiave, tobia_libero, custode, custode_stunned, campanello_usato, canto, voce_ferma_scade_round
        for n in vivi():
            h = HERO[n]
    ...
    for tappa in path:
        ...
        if tappa.startswith('T3') and 'ritorno' not in tappa:
            # La Galleria delle Eco (la trappola Difficile dell'episodio):
            # con LA CANNA MUTA nessuna prova; col Presagio di L7 letto la
            # prova scende a Media; altrimenti NERVI (Difficile) per ogni
            # eroe. Chi fallisce: 1 sola azione al prossimo turno.
            if indagine.get('canna'):
                log('    [OGGETTO] LA CANNA MUTA al petto: la galleria non vi sente — nessuna prova.')
            else:
                diff_galleria = 'Media' if (7, 'Presagio') in indagine.get('approf_dettaglio', set()) else 'Difficile'
                if diff_galleria == 'Media':
                    log('    [HOOK INDAGINE] Il Presagio della sacrestia: tacete e stringete qualcosa al petto — prova declassata a Media.')
                for n in vivi():
    ...
    if esito is None and custode and custode['fer'] > 0:
        ...
        while custode['fer'] > 0 and vivi():
            round_n += 1
            attivati_extra.clear()
            log(f'--- Round {round_n}: scontro nel Pozzo Maestro ---')
            log_azioni_round()
            fase_eroi('T6')
            fase_minaccia()
    ...
    log(f'Accordatore svegliato: {"in anticipo al round " + str(round_custode_svegliato) if round_custode_svegliato else "solo a T6 (mai in anticipo via Canto)"}')
    for n in party:
...
def esegui_batch(nome_base, party, seeds, formula_minaccia='standard', nemico_scale='nessuna',
    ...
    pct_diapason = sum(1 for i in ind_list if i.get('campanello')) / n * 100
    pct_canna = sum(1 for i in ind_list if i.get('canna')) / n * 100
...
def sessione_curva():
    """Curva completa 2-10 dell'Episodio 3 sulla config di produzione
    (formule riusate 1:1, vedi bibbia: si rilancia la curva sul nuovo
    episodio perche' mappa/boss/mazzo diversi = affollamento diverso).
    Seed 620000+ (mai usati altrove)."""
    ...
    for size in (2, 3, 4, 5, 6, 7, 8, 9, 10):
        ...
        m = esegui_batch_multi_party(nome, size, 'finale_v3', 'nessuna',
                                      n_party=5, n_seed=30, seed_base=620000 + size * 1000)
        risultati.append(m)

    path = os.path.join(LOG_DIR, 'riepilogo_ep3.md')
    with open(path, 'w', encoding='utf-8') as f:
        f.write('# Episodio 3 — curva 2-10, config di produzione riusata\n\n')
    ...
    print(f'\nCurva Episodio 3 fatta. Riepilogo in {path}')
