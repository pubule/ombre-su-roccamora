# -*- coding: utf-8 -*-
"""Simulatore di playtest per Ombre su Roccamora - EPISODIO 2 «La voce del bronzo».

COPIA di simulate_playtest.py (il riferimento storico dell'Ep. 1, da non
toccare) cucita sull'Episodio 2 come da bibbia: mappa nuova (TILES_2 di
gen_ep2), boss LO SCORIATORE, mazzo Minaccia proprio, luoghi/chiavi del
caso del bronzo. Le formule di scaling (MINACCIA_FORMULE, CUSTODE_
TENSIONE_EXTRA, SALUTE_BONUS_PER_N, TICK_CANTO_OGNI/SOGLIA_CANTO) sono
RIUSATE identiche - config di produzione 20260716.
Novita' meccanica unica: il Crogiolante attacca fino a 2 caselle in
linea (campo `gittata`). Non simulati e dichiarati: le 2 carte Segugi
del Coro (variante Bivio), il badile/la latta da Cercare, il contrassegno
e' modellato come "trovato se L7 visitato".
...
from gen_ep2 import TILES_2 as TILES, NEMICI_2, LUOGHI_2  # noqa: E402

NEMICI = NEMICI_COMUNI + NEMICI_2
HERO = {h['nome']: h for h in HEROES}
LUOGHI_BY_N = {l['n']: l for l in LUOGHI_2}

# Mazzo Minaccia dell'episodio (21 carte - le 2 «Segugi del Coro» della
# variante Bivio NON sono simulate, dichiarato nel docstring). Tuple
...
CHI_ESPLICITO = {(1, 'Osservazione'), (3, 'Testimonianza'), (6, 'Testimonianza')}
...
TOKEN_POOL_BASE = {'ADEPTO INCAPPUCCIATO': 4, 'IL CROGIOLANTE': 4,
                    'LO SGHERRO': 6, 'IL SICARIO': 2}
...
CARD_SPAWN = {
    'TURNO DI GUARDIA': ('LO SGHERRO', 1, False),
    'I BRAVI DEL CAPOMASTRO': ('LO SGHERRO', 1, False),
    'FISCHIO DAL PIAZZALE': ('LO SGHERRO', 1, False),
    'IL GIRO DEL PIAZZALE': ('LO SGHERRO', 2, False),
    'IL CARCERIERE': ('IL SICARIO', 1, False),
    'LAMA TRA LE FORME': ('IL SICARIO', 1, True),
    'MESTOLATA': ('IL CROGIOLANTE', 1, False),
    'IL SECCHIO BOLLE': ('IL CROGIOLANTE', 1, False),
    'SQUADRA DI COLATA': ('IL CROGIOLANTE', 1, False),
    'IL RITIRO': ('ADEPTO INCAPPUCCIATO', 1, False),
    'VOLTI ALLA BANCHINA': ('ADEPTO INCAPPUCCIATO', 1, False),
}
# Stessa semantica dell'Ep. 1 (vedi simulate_playtest.py): DISTANZA_PORTA =
# "sull'uscita piu' vicina agli eroi", None = inseguitori dall'ingresso
# della Banchina (distanza dinamica).
DISTANZA_PORTA = 1
SPAWN_DISTANZA = {
    'TURNO DI GUARDIA': DISTANZA_PORTA,
    'I BRAVI DEL CAPOMASTRO': DISTANZA_PORTA,
    'FISCHIO DAL PIAZZALE': DISTANZA_PORTA,
    'IL GIRO DEL PIAZZALE': None,
    'IL CARCERIERE': DISTANZA_PORTA,
    'MESTOLATA': DISTANZA_PORTA,
    'IL SECCHIO BOLLE': DISTANZA_PORTA,
    'SQUADRA DI COLATA': DISTANZA_PORTA,
    'IL RITIRO': None,
    'VOLTI ALLA BANCHINA': None,
}
INSIDIA = {  # titolo -> (difficolta', danno, chi prova)
    'CENERE NEGLI OCCHI': ('Media', 1, 'l’eroe più avanzato'),
    'PAVIMENTO DI SCORIA': ('Media', 1, 'l’eroe attivo'),
    'IL FISCHIO DEL FORNO': ('Facile', 0, 'ogni eroe (chi fallisce: 1 sola azione prossimo turno)'),
    'IL RONZIO NEI DENTI': ('Media', 1, 'l’eroe con meno NERVI'),
}
# Insidie il cui fallimento costa anche 1 azione (CENERE e RONZIO no).
PERDE_AZIONE = {'PAVIMENTO DI SCORIA', 'IL FISCHIO DEL FORNO'}
CRESCENDO = {'IL PRIMO RINTOCCO', 'IL BRONZO RISPONDE', 'LA LEGA CANTA'}
MALAVITA_TRUPPA = {'LO SGHERRO', 'IL SICARIO', 'ADEPTO INCAPPUCCIATO', 'IL CROGIOLANTE'}

# Il boss dell'episodio (vedi gen_ep2.NEMICI_2 e DESIGN-EPISODIO-2.md):
# Dif -1 e Fer +1 rispetto al Custode - piu' facile colpirlo, piu' lungo
# abbatterlo. La "stonatura" (smorzo+martello) fa Dif 8->5 + salta
# un'attivazione: stessa matematica del diapason.
CUSTODE = dict(nome='LO SCORIATORE', att=3, dif=8, fer=4, mov=3, dan=2)
...
LUOGHI_SIM = [
    # Ep. 2: chiavi garantite negli indizi core (gen_ep2.LUOGHI_2), tutte da
    # luoghi aperti (1, 2, 3, 6). `stona=True` sui due pezzi della debolezza
    # del boss (martello L1 + smorzo L2): servono ENTRAMBI (vedi sotto).
    dict(n=1, nome='La Fonderia Dossena', req=None, sblocca_parola='GHISA DA SCAFO', chiude=None,
         approf=['Osservazione', 'Referto'], stona=True),
    dict(n=2, nome='La Cella Campanaria di San Teodoro', req=None, sblocca_parola='SENZA LANTERNE',
         chiude=None, approf=['Presagio', 'Testimonianza'], stona=True),
    dict(n=3, nome='L’Osteria della Bilancia', req=None,
         sblocca_parola=('PARI PESO', 'SENZA LANTERNE'), chiude=None,
         approf=['Testimonianza']),
    dict(n=4, nome='Il Deposito Daziario', req=('parola', 'PARI PESO'), chiude=None,
         approf=['Referto', 'Osservazione']),
    dict(n=5, nome='Corte della Faenza', req=('oggetto', 'LA POLIZZA DEL MONTE'), chiude=None,
         approf=['Testimonianza']),
    dict(n=6, nome='Il Banco dei Pegni di Fossa', req=None, sblocca_parola='PARI PESO',
         sblocca_oggetto='LA POLIZZA DEL MONTE', chiude=None,
         approf=['Testimonianza']),
    dict(n=7, nome='Il Molo delle Chiatte', req=('parola', 'SENZA LANTERNE'), chiude=21,
         approf=['Presagio'], contrassegno=True),
    dict(n=8, nome='La Camera dei Pesi e delle Misure', req=('parola', 'PARI PESO'), chiude=22,
         approf=['Referto']),
    dict(n=9, nome='Il Cimitero delle Barche', req=('parola', 'GHISA DA SCAFO'), chiude=None,
         approf=['Osservazione']),
]
...
def simula_indagine(party, log, esplora_a_fondo=False):
    ...
    parole = set()
    oggetti = set()
    stona_pezzi = 0   # martello (L1) + smorzo (L2): servono entrambi
    contrassegno = False  # L7: sbarco silenzioso in spedizione
    approf_letti = 0
    approf_falliti = 0
    ...
    def punteggio(l):
        ...
        # Ep. 2: strutturali = le 4 fonti di chiavi, tutte aperte (L1/L2/L3/L6).
        # Niente pista-diapason: i due pezzi della stonatura stanno negli
        # indizi core di L1/L2, gia' strutturali.
        strutturale = 0 if l['n'] in (1, 2, 3, 6) else 1
    ...
    while ore > 0:
        ...
        if l.get('stona'):
            stona_pezzi += 1
            log(f'    -> Trovato un pezzo della stonatura ({stona_pezzi}/2: martello di collaudo + smorzo di feltro).')
        if l.get('contrassegno'):
            contrassegno = True
            log('    -> Trovato: IL CONTRASSEGNO DI PIOMBO (sbarco silenzioso alla Banchina).')
        if ok:
    ...
    log(f'Stonatura completa (smorzo+martello): {"sì" if stona_pezzi >= 2 else "no"}; '
        f'contrassegno: {"sì" if contrassegno else "no"}')
    ...
    log('')
    return dict(ore_avanzate=ore_avanzate, tier=tier, diapason=stona_pezzi >= 2,
                contrassegno=contrassegno, visitati=visitati,
...
def simula_spedizione(party, indagine, log, run_seed, formula_minaccia='standard',
    ...
    log('=' * 78)
    log(f'Party: {", ".join(party)}')
    log(f'Bonus da Indagine: {indagine["tier"]}; stonatura (smorzo+martello): '
        f'{"sì" if indagine["diapason"] else "no"}; contrassegno: '
        f'{"sì" if indagine.get("contrassegno") else "no"}')
    log('NOTA: gruppo trattato come blocco unico tessera per tessera (vedi intestazione script).')
    # Con la stonatura in mano (smorzo + martello) il gruppo VA ADDOSSO al boss:
    # e' l'unica ragione per cui ha passato l'Indagine a cercarla. Senza, non
    # ingaggia e punta al condotto. Serve anche a scegliere il bersaglio: la
    # vecchia euristica «il piu' vicino, poi il piu' debole» non attaccava MAI
    # lo Scoriatore — ha piu' ferite di ogni gregario e la sala si riempie a
    # ogni round, quindi restava schermato per sempre e il combattimento non
    # finiva (misurato: 17 round di scontro, ZERO attacchi al boss).
    affronta_boss = bool(indagine.get('diapason'))
    ...
    # Domanda 3: senza il Contrassegno di Piombo (L7), lo sbarco sveglia
    # la guardia - 2 Sgherri gia' in T1 (testo tessera, QUANDO RIVELATE).
    sbarco_rumoroso = not indagine.get('contrassegno')
    if pool_extra:
    ...
    if sbarco_rumoroso:
        ...
        log('  Sbarco senza contrassegno: 2 Sgherri appaiono tra le scorie della Banchina.')
    ...
    # La passerella e' un corridoio stretto sull'acqua: si attraversa in
    # fila indiana, DUE round (fedele alla geometria della tessera - e
    # senza, la spedizione Ep.2 durava 11 round contro i 15 dell'Ep.1:
    # curva 91-97% alle taglie 6-10, troppo comoda).
    path = ['T2 (Piazzale delle Forme)', 'T4 (Passerella sul canale di scolo)',
            'T4 (Passerella — fila indiana)',
            'T5 (Ufficio del Pesatore)', 'T6 (Sala dei Forni)']
    round_n = 0
    esito = None
    ...
    def aggiungi_canto(cura_custode=False):
        ...
        if custode is None:
            if canto >= SOGLIA_CANTO:
                ...
                for _ in range(1):
                    if pool['IL CROGIOLANTE'] <= 0:
                        pool_esauriti_totale += 1
                        log('    Segnalini IL CROGIOLANTE esauriti: lo Scoriatore si desta senza scorta.')
                        continue
                    pool['IL CROGIOLANTE'] -= 1
                    base = NEMICO['IL CROGIOLANTE']
    ...
    def fase_eroi(luogo_label):
        ...
        for n in vivi():
            ...
            if custode and custode['fer'] > 0 and diapason['has'] and not diapason_usato and not custode_stunned:
                diapason_usato = True
                log(f'    [OGGETTO] {n} batte il martello smorzato sul petto dello Scoriatore: '
    ...
    for tappa in path:
        ...
        if tappa.startswith('T4') and round_n:
            # La passerella: NERVI (Facile) o 1 sola azione. Hook Indagine->
            # Spedizione (doppia via): Presagio L2 o Osservazione L9 = niente prova.
            if ((2, 'Presagio') in indagine.get('approf_dettaglio', set())
                    or (9, 'Osservazione') in indagine.get('approf_dettaglio', set())):
                log('    [HOOK INDAGINE] Le assi rattoppate non vi sorprendono: passate piano, niente prova.')
            else:
                for n in vivi():
        ...
        if tappa.startswith('T5') and not ruggero_libero:
            log('    QUANDO RIVELATE: il Sicario appare accanto alla porta (il carceriere).')
            ...
            log('    Ilario Dossena è legato alla sedia: liberato con Interagire (nessuna prova).')
            ruggero_libero = True
        if tappa.startswith('T6') and custode is None:
            ...
            log(f'    Rivelata la Cripta della Cera: lo Scoriatore si desta con 1 Crogiolante, '
    ...
    # AFFRONTARLO O EVITARLO. Il fascicolo da' due finali: si vince anche
    # quando Ilario entra nel condotto, senza abbattere lo Scoriatore. Prima
    # questo ciclo inchiodava il gruppo al boss finche' uno dei due moriva —
    # 22 round di scontro, 51 carte pescate, 24 nemici piazzati contro 19
    # abbattuti: un tapis roulant, non un combattimento.
    # Il gruppo ingaggia SOLO se ha di che stonarlo (smorzo + martello, presi
    # in Indagine): e' la scelta che quella debolezza esiste per rendere
    # possibile. Senza, prende Ilario e cerca il condotto.
    affronta = bool(indagine.get('diapason'))
    if esito is None and custode and custode['fer'] > 0 and not affronta:
        log('--- Senza la stonatura non si ingaggia: si prende Ilario e si cerca il condotto ---')
    if esito is None and custode and custode['fer'] > 0 and affronta:
...
def sessione_curva():
    """Curva completa 2-10 dell'Episodio 2 sulla config di produzione
    (formule riusate 1:1 dall'Ep. 1, vedi bibbia: si rilancia la curva sul
    nuovo episodio perche' mappa/boss/mazzo diversi = affollamento diverso).
    Seed 610000+ (mai usati altrove). Motore deterministico (tie-break
    ordinati, eredita' del fix 20260716)."""
    ...
    for size in (2, 3, 4, 5, 6, 7, 8, 9, 10):
        ...
        m = esegui_batch_multi_party(nome, size, 'finale_v3', 'nessuna',
                                      n_party=5, n_seed=30, seed_base=610000 + size * 1000)
        risultati.append(m)

    path = os.path.join(LOG_DIR, 'riepilogo_ep2.md')
    with open(path, 'w', encoding='utf-8') as f:
        f.write('# Episodio 2 — curva 2-10, config di produzione riusata\n\n')
    ...
    print(f'\nCurva Episodio 2 fatta. Riepilogo in {path}')
