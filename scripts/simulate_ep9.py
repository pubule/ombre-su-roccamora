# -*- coding: utf-8 -*-
"""Simulatore di playtest - EPISODIO 9 «Il processo».

COPIA di simulate_ep7.py cucita sull'Episodio 9 (vedi DESIGN-EPISODIO-9.md):
obiettivo SCORTA — il teste RIVA (PNG fragile: 3 Salute, Mov 3, non
combatte) va portato vivo dalla sacrestia (T1) al Molo del Lume (T6),
percorso lineare a 6 tessere con imboscate. Gli aggressori bersagliano
Riva; IL SICARIO GENTILE (boss, appare in T3) caccia il teste (se puo'
raggiungerlo attacca lui, Mov 4). Vittoria = Riva a T6; Riva morto =
sconfitta. Leve: Mantella (primo colpo a Riva in T4 manca), Fischietto
(-1 spawn una volta), Salvacondotto (salta una tessera d'imboscata),
D2-esita, +1 Danno del Sicario col ramo Ferri-vivo. Nomi interni riusati:
`libretto` = SALVACONDOTTO (D4), `pianta` = MANTELLA (T4). Config di
produzione condivisa riusata. Seed base 690000.
"""
...
from gen_ep9 import TILES_9 as TILES, NEMICI_9, LUOGHI_9  # noqa: E402

NEMICI = NEMICI_COMUNI + NEMICI_9
HERO = {h['nome']: h for h in HEROES}
LUOGHI_BY_N = {l['n']: l for l in LUOGHI_9}
...
MINACCE = (
    [('I BRAVI DEL FONDO', 'Piazzate 1 Sgherro sull’uscita più vicina agli eroi.', 'malavita', False),
     ...
     ('IL BRACCIO DELL’AVVOCATO', 'Piazzate 1 Sgherro: si attiva subito.', 'malavita', True),
     ('I COLLEGHI DEL GENTILE', 'Piazzate 1 Sicario sull’uscita più vicina agli eroi.', 'malavita', False),
     ('IL COLTELLO DAL BUIO', 'Piazzate 1 Sicario: si attiva subito.', 'malavita', True),
     ('LA FOLLA COMPRATA', 'Piazzate 1 Sgherro sull’ingresso della tessera.', 'malavita', False),
     ('GLI UBRIACHI SU ORDINAZIONE', 'Piazzate 1 Sgherro sull’ingresso della tessera.', 'malavita', False),
     ('IL CAPANNELLO', 'Piazzate 1 Sgherro sull’uscita più vicina agli eroi.', 'malavita', False),
     ('LA RETROGUARDIA', 'Piazzate 1 Sgherro sull’ingresso della tessera.', 'malavita', False),
     ('L’UOMO SUL TETTO', 'Piazzate 1 Sicario sull’uscita più vicina agli eroi.', 'malavita', False),
     ('IL FISCHIO DI RICHIAMO', 'Piazzate 1 Sgherro: si attiva subito.', 'malavita', True),
     ('LA NEBBIA DEL FIUME', 'Ogni eroe prova NERVI (Facile): chi fallisce ha 1 sola azione al prossimo turno.', 'insidia', False),
     ('IL SELCIATO VISCIDO', 'L’eroe più avanzato prova NERVI (Media): se fallisce, 1 danno.', 'insidia', False),
     ('LA FOLLA CHE SPINGE', 'L’eroe adiacente a Riva prova NERVI (Media): se fallisce, Riva è spinto.', 'insidia', False),
     ('LA PRIMA RONDA PASSA', 'Aggiungete 1 segnalino Canto (l’Ora che stringe).', 'crescendo', False),
     ('LE CAMPANE DELL’UNA', 'Aggiungete 1 segnalino Canto (l’Ora che stringe).', 'crescendo', False),
     ('L’INTERVALLO DELLE RONDE', 'Aggiungete 1 segnalino Canto (l’Ora che stringe).', 'crescendo', False),
     ('UN PORTONE AMICO', 'Nessun effetto (tensione).', 'quiete', False),
     ('LA SCORCIATOIA DI RANUZZI', 'Rivelate una tessera coperta adiacente.', 'favore', False),
     ('IL PONTE LEVATO', 'Muoversi costa il doppio sulla tessera dell’eroe attivo fino a fine round.', 'ostacolo', False),
     ('LA LAMA EDUCATA', 'L’eroe adiacente a Riva subisce 1 danno (fare da scudo ha un prezzo).', 'insidia', False)])
# Rivelatorio Domanda 2 (Voltan), 3 carte in aperti: L2-Osservazione,
# L3-Testimonianza, L4-Testimonianza.
CHI_ESPLICITO = {(2, 'Testimonianza'), (3, 'Testimonianza'), (4, 'Testimonianza')}
...
TOKEN_POOL_BASE = {'LO SGHERRO': 5, 'IL SICARIO': 3}
...
CARD_SPAWN = {
    'I BRAVI DEL FONDO': ('LO SGHERRO', 1, False),
    'LA CARITÀ ARMATA': ('LO SGHERRO', 1, False),
    'IL BRACCIO DELL’AVVOCATO': ('LO SGHERRO', 1, True),
    'I COLLEGHI DEL GENTILE': ('IL SICARIO', 1, False),
    'IL COLTELLO DAL BUIO': ('IL SICARIO', 1, True),
    'LA FOLLA COMPRATA': ('LO SGHERRO', 1, False),
    'GLI UBRIACHI SU ORDINAZIONE': ('LO SGHERRO', 1, False),
    'IL CAPANNELLO': ('LO SGHERRO', 1, False),
    'LA RETROGUARDIA': ('LO SGHERRO', 1, False),
    'L’UOMO SUL TETTO': ('IL SICARIO', 1, False),
    'IL FISCHIO DI RICHIAMO': ('LO SGHERRO', 1, True),
}
DISTANZA_PORTA = 1
SPAWN_DISTANZA = {
    'I BRAVI DEL FONDO': DISTANZA_PORTA,
    'LA CARITÀ ARMATA': DISTANZA_PORTA,
    'IL BRACCIO DELL’AVVOCATO': DISTANZA_PORTA,
    'I COLLEGHI DEL GENTILE': DISTANZA_PORTA,
    'IL COLTELLO DAL BUIO': DISTANZA_PORTA,
    'LA FOLLA COMPRATA': None,
    'GLI UBRIACHI SU ORDINAZIONE': None,
    'IL CAPANNELLO': DISTANZA_PORTA,
    'LA RETROGUARDIA': None,
    'L’UOMO SUL TETTO': DISTANZA_PORTA,
    'IL FISCHIO DI RICHIAMO': DISTANZA_PORTA,
}
INSIDIA = {  # titolo -> (difficolta', danno, chi prova)
    'IL SELCIATO VISCIDO': ('Media', 1, 'l’eroe più avanzato'),
    'LA NEBBIA DEL FIUME': ('Facile', 0, 'ogni eroe (chi fallisce: 1 sola azione prossimo turno)'),
    'LA FOLLA CHE SPINGE': ('Media', 0, 'l’eroe adiacente a Riva'),
}
PERDE_AZIONE = {'LA NEBBIA DEL FIUME'}
CRESCENDO = {'LA PRIMA RONDA PASSA', 'LE CAMPANE DELL’UNA', 'L’INTERVALLO DELLE RONDE'}
MALAVITA_TRUPPA = {'LO SGHERRO', 'IL SICARIO'}
# «La lama educata»: danno a un eroe adiacente a Riva (il prezzo dello scudo).
DANNO_SCUDO = {'LA LAMA EDUCATA'}
DANNO_DIRETTO = set()  # nessun danno-diretto senza prova in Ep.9

# Il boss dell'episodio (vedi gen_ep9.NEMICI_9 e DESIGN-EPISODIO-9.md):
# CACCIA il teste (Mov 4). Nessuna debolezza-oggetto. «Il nome sbagliato»
# (D2 esatta): salta la prima attivazione. Col ramo Ferri-vivo: +1 Danno
# la prima volta che raggiunge Riva.
CUSTODE = dict(nome='IL SICARIO GENTILE', att=3, dif=8, fer=4, mov=4, dan=2)
...
LUOGHI_SIM = [
    # Ep. 9: chiavi garantite negli indizi core, tutte da aperti (1-4).
    # `libretto=True` = L1 consegna il SALVACONDOTTO (D4); `pianta=True` =
    # L6 (la Mantella: primo colpo a Riva in T4 manca); `incrocio_d1` =
    # riscontri D1 (L1/L4, ne servono 2); `incrocio_d3` = riscontri D3
    # (L3/L5, ne servono 2). `apre`/`chiude` = vincoli d'orario.
    dict(n=1, nome='Il Tribunale', req=None, chiude=20,
         sblocca_parola='LA DEPOSIZIONE DI DOMANI',
         sblocca_oggetto='IL SALVACONDOTTO DEL GIUDICE',
         approf=['Osservazione'], incrocio_d1=True, libretto=True),
    dict(n=2, nome='La Redazione della Gazzetta', req=None,
         sblocca_parola=('IL FONDO CARITATEVOLE', 'L’ORO DELLA PARCELLA'), chiude=None,
         approf=['Testimonianza']),
    dict(n=3, nome='La Pensione del Giurato', req=None,
         sblocca_parola=('L’ORO DELLA PARCELLA', 'IL FONDO CARITATEVOLE'), chiude=None,
         approf=['Testimonianza'], incrocio_d3=True),
    dict(n=4, nome='La Gendarmeria', req=None,
         sblocca_parola=('LA DEPOSIZIONE DI DOMANI', 'IL FORESTIERO COI GUANTI'), chiude=None,
         approf=['Testimonianza'], incrocio_d1=True),
    dict(n=5, nome='Lo Studio dell’Avvocato Grassi', req=('parola', 'IL FONDO CARITATEVOLE'),
         chiude=19, approf=['Referto'], incrocio_d3=True),
    dict(n=6, nome='La Sacrestia del Tribunale', req=('parola', 'LA DEPOSIZIONE DI DOMANI'),
         chiude=None, approf=['Osservazione'], pianta=True),
    dict(n=7, nome='La Casa del Teste', req=('parola', 'L’ORO DELLA PARCELLA'),
         chiude=None, approf=['Presagio']),
    dict(n=8, nome='La Locanda del Forestiero', req=('parola', 'IL FORESTIERO COI GUANTI'),
         chiude=23, approf=['Referto']),
    dict(n=9, nome='L’Approdo della Società', req=('oggetto', 'IL SALVACONDOTTO DEL GIUDICE'),
         chiude=None, approf=['Presagio']),
]
...
def simula_indagine(party, log, esplora_a_fondo=False):
    log('=' * 78)
    log('INDAGINE - Episodio 9: "Il processo"')
    log('=' * 78)
    ...
    parole = set()
    oggetti = set()
    pianta = False       # L6: la Mantella da Sacrestano (primo colpo a Riva in T4 manca)
    libretto = False     # L1: il SALVACONDOTTO (Domanda 4, salta una tessera d’imboscata)
        ...
        for tipo in l['approf']:
            ...
            if idoneo:
                ...
                if (l['n'], tipo) in CHI_ESPLICITO:
                    chi_confermato = True
                    log('    -> Questa carta conferma esplicitamente che a pagare l’avvocato è la mano dell’oro vecchio (Domanda 2).')
            else:
    ...
    while ore > 0:
        ...
        if l.get('pianta'):
            pianta = True
            log('    -> Trovata: LA MANTELLA DA SACRESTANO (nel Mercato il primo colpo a Riva manca).')
        if l.get('libretto'):
            libretto = True
            log('    -> Trovato: IL SALVACONDOTTO DEL GIUDICE (salta una tessera d’imboscata — Domanda 4).')
        if l.get('incrocio_d1'):
            incroci_d1 += 1
            log(f'    -> Riscontro sulla Domanda 1 ({incroci_d1}: ruolo d’udienza/usciere).')
        if l.get('incrocio_d3'):
            incroci_d3 += 1
            log(f'    -> Riscontro sulla Domanda 3 ({incroci_d3}: registro ronde/giurato).')
        if ok:
    ...
    d1_ok = incroci_d1 >= 2
    d3_ok = incroci_d3 >= 2
    log(f'Salvacondotto: {"sì" if libretto else "no"}; '
        f'Mantella: {"sì" if pianta else "no"}; Domanda 1 ({incroci_d1} riscontri): '
...
def simula_spedizione(party, indagine, log, run_seed, formula_minaccia='standard',
    ...
    log('=' * 78)
    log('SPEDIZIONE - La scorta del teste, dopo mezzanotte')
    log('=' * 78)
    log(f'Party: {", ".join(party)}')
    ...
    if custode_extra_fer:
        log(f'Tensione tavolo piccolo: +{custode_extra_fer} Ferite SOLO al Sicario Gentile (non ai nemici di truppa).')
    log('')
    ...
    # Domanda 1 sbagliata: si cerca Riva a tentoni - 1 Sgherro in T1.
    # Domanda 3 sbagliata: si parte tardi (ronde ritirate) - +1 Canto.
    ...
    if sbarco_rumoroso:
        ...
        log('  Si cerca Riva a tentoni (Domanda 1): 1 Sgherro appare alla sacrestia.')
    ...
    # RIVA (il teste scortato): PNG fragile, 3 Salute, si muove col gruppo,
    # non combatte. Gli aggressori lo bersagliano; se cade, scorta fallita.
    riva = {'salute': 3, 'salute_max': 3, 'pos': None, 'vivo': True}
    riva_scudo_round = [-1]     # round dell'ultimo ricalcolo degli scudi
    riva_scudi_disp = [0]      # scudi (guardie del corpo) ancora disponibili nel round
    mantella = indagine.get('pianta')   # T4: primo colpo a Riva manca
    mantella_usata = [False]
    fischietto = [indagine.get('libretto') is not None]  # sempre disponibile: -1 spawn una volta (semplificazione)
    fischietto[0] = True
    ferri_vivo = [indagine.get('ferri_vivo', False)]     # +1 Danno del Sicario la 1a volta su Riva
    sicario_bonus_speso = [False]
    canto = 0 if indagine.get('d3_ok') else 1
    if not indagine.get('d3_ok'):
        log('  Partenza tardi, ronde già ritirate (Domanda 3): +1 segnalino Canto.')
    if indagine.get('libretto'):
        log('  Col Salvacondotto (Domanda 4): alla partenza si salta una tessera d’imboscata.')
    ...
    def aggiungi_canto(cura_custode=False):
        """+1 segnalino Canto (l'ORA CHE STRINGE); alla soglia, pesca extra
        per sempre. Il Sicario Gentile NON si desta via Canto (di copione,
        a T3); `cura_custode` = "il Sicario si attiva subito" (crescendo)."""
        nonlocal canto, custode, custode_stunned, pool_esauriti_totale, canto_bonus_carte, round_custode_svegliato
        canto += 1
        log(f'    Segnalino Canto (l’Ora che stringe): {canto}.')
        if canto >= SOGLIA_CANTO and not canto_bonus_carte:
            canto_bonus_carte = True
            log(f'    L’Ora raggiunge {SOGLIA_CANTO}: l’intervallo delle ronde è al colmo — '
                'da ora ogni Fase Minaccia pesca 1 carta in più (fino a fine spedizione).')
        if cura_custode and custode and custode['fer'] > 0 and custode.get('pos') and (vivi() or riva['vivo']):
            log('    Il Sicario Gentile si attiva subito (l’ora lo chiama):')
    ...
    def fase_minaccia():
        ...
        for _ in range(n_carte):
            ...
            elif titolo in DANNO_SCUDO:
                # «La lama educata»: 1 danno a un eroe (il prezzo dello scudo)
                if vivi():
                    b = random.choice(vivi())
                    applica_danno(b, 1, titolo)
            elif titolo == 'LA FOLLA CHE SPINGE':
                # Riva spinto: se il Sicario e' vicino, rischia — qui 1 danno
                # diretto a Riva con prob. bassa (la folla lo espone)
                if riva['vivo'] and random.random() < 0.35:
                    riva['salute'] -= 1
                    log(f'    [FOLLA] Riva è spinto allo scoperto: 1 danno (Salute {max(riva["salute"],0)}/3).')
                    if riva['salute'] <= 0:
                        riva['vivo'] = False
                        log('    *** RIVA È CADUTO nella calca. La scorta è fallita. ***')
                else:
                    log('    [FOLLA] La folla spinge, ma Riva resta coperto dal gruppo.')
    ...
    def _colpisci_riva(e):
        """L'aggressore adiacente a Riva lo colpisce, salvo scudo (un eroe
        adiacente a Riva intercetta il PRIMO colpo del round) e salvo la
        Mantella (in T4, il primo colpo a Riva manca)."""
        nonlocal custode_stunned
        if mantella and tile_attuale == 'T4' and not mantella_usata[0]:
            mantella_usata[0] = True
            log(f'  {e["nome"]} scambia Riva per un manichino (Mantella): il colpo va a vuoto.')
            return
        # ogni eroe entro 1 cella da Riva puo' parare UN colpo per round:
        # piu' guardie del corpo = piu' scudo (la taglia del party conta)
        if riva_scudo_round[0] != round_n:
            riva_scudo_round[0] = round_n
            riva_scudi_disp[0] = sum(1 for n in vivi()
                if abs(pos[n][0] - riva['pos'][0]) + abs(pos[n][1] - riva['pos'][1]) <= 1)
        riva_dif = 8
        if riva_scudi_disp[0] > 0:
            riva_scudi_disp[0] -= 1
            difensore = min((n for n in vivi()
                             if abs(pos[n][0] - riva['pos'][0]) + abs(pos[n][1] - riva['pos'][1]) <= 1),
                            key=lambda m: salute[m], default=None)
            if difensore and e is not custode:
                # truppa: la guardia para del tutto il colpo (lo prende lei)
                log(f'  {difensore} fa da SCUDO a Riva e para il colpo di {e["nome"]}:')
                if enemy_attack_roll(log, e['nome'], e['att'], difensore, 8):
                    applica_danno(difensore, e['dan'], e['nome'])
                return
            if difensore:
                # boss (un professionista): la guardia non blocca, ma copre
                # Riva — +2 Difesa contro il Sicario per questo colpo
                riva_dif = 10
                log(f'  {difensore} copre Riva dal Sicario (Riva +2 Difesa, non si para del tutto un professionista).')
        dan = e['dan']
        if e is custode and ferri_vivo[0] and not sicario_bonus_speso[0]:
            sicario_bonus_speso[0] = True
            dan += 1
            log('  [FERRI VIVO] Il Sicario colpisce Riva con particolare cura (+1 Danno).')
        log(f'  {e["nome"]} punta RIVA (il teste):')
        if enemy_attack_roll(log, e['nome'], e['att'], 'RIVA', riva_dif):
            riva['salute'] -= dan
            log(f'    RIVA subisce {dan} danno (Salute: {max(riva["salute"], 0)}/3).')
            if riva['salute'] <= 0:
                riva['vivo'] = False
                log('    *** RIVA È CADUTO. La scorta è fallita. ***')

    def _avvicina_e_attacca(e, difesa):
        """Scorta: gli aggressori (e il Sicario Gentile) puntano RIVA se
        possono raggiungerlo; altrimenti un eroe. Muove verso Riva se vivo,
        poi colpisce Riva se adiacente (scudo/Mantella permettendo), oppure
        l'eroe adiacente."""
        vivi_ora = vivi()
        caccia_riva = riva['vivo'] and riva['pos'] is not None
        # il boss caccia SEMPRE Riva; la truppa lo punta se e' un bersaglio
        # sensato (adiacente o raggiungibile), altrimenti l'eroe piu' vicino
        meta = riva['pos'] if caccia_riva else pos[random.choice(vivi_ora)]
        nuova = muovi_verso(tile_attuale, e['pos'], meta, e['mov'], celle_occupate(esclusa=e))
        if nuova != e['pos']:
            dest = 'Riva' if caccia_riva else 'il gruppo'
            log(f'  {e["nome"]} si muove verso {dest}: {chess(e["pos"])} -> {chess(nuova)}.')
            e['pos'] = nuova
        # Riva adiacente? il boss e gli aggressori lo colpiscono
        if caccia_riva and (abs(e['pos'][0] - riva['pos'][0]) + abs(e['pos'][1] - riva['pos'][1]) <= 1):
            _colpisci_riva(e)
            return
        ...
        if not adiacenti_ora:
            ...
            return
        bersaglio_reale = random.choice(adiacenti_ora)
                ...
                if attack_roll(log, n, h['vigore'], armed[n], bersaglio_e['nome'], bersaglio_e['dif']):
                    ...
                    if bersaglio_e['fer'] <= 0:
                        ...
                        if bersaglio_e is custode:
                            log('    *** IL SICARIO GENTILE È A TERRA. ***')
                        else:
    ...
    # PERCORSO lineare della scorta. Col Salvacondotto (D4) si salta UNA
    # tessera d'imboscata (T2/T4/T5): la piu' pericolosa raggiungibile.
    path = ['T1 (Sacrestia)', 'T2 (Vicolo dei Tintori)', 'T3 (Ponte delle Catene)',
            'T4 (Mercato Coperto)', 'T5 (Salita dei Lampionai)', 'T6 (Molo del Lume)']
    if indagine.get('libretto'):
        # salta T4 (il Mercato, imboscata doppia) se non abbiamo la Mantella;
        # altrimenti salta T2 (la Mantella copre proprio T4)
        salta = 'T2 (Vicolo dei Tintori)' if mantella else 'T4 (Mercato Coperto)'
        path = [t for t in path if t != salta]
    ...
    for tappa in path:
        ...
        riva['pos'] = porta_attuale_pos   # Riva si muove col gruppo (Mov 3)
        ...
        if tile_id == 'T2':
            ...
            for _ in range(_n_imb):
                if pool['LO SGHERRO'] <= 0:
                    ...
                    break
                pool['LO SGHERRO'] -= 1
                base = NEMICO['LO SGHERRO']; a_fer = base['fer'] + fer_bonus
                cella = cella_libera_vicino(tile_id, porta_attuale_pos, celle_occupate())
                enemies.append(dict(nome='LO SGHERRO', fer=a_fer, fer_max=a_fer,
                                     dif=base['dif'], att=base['att'], dan=base['dan'] + dan_bonus,
                                     mov=base['mov'], pos=cella or porta_attuale_pos))
        if tile_id == 'T3' and custode is None:
            log('    Sul Ponte delle Catene, appare IL SICARIO GENTILE: si toglie il '
                'cappello. «Signori. Mi dispiace per quello che segue.»')
            c_fer = CUSTODE['fer'] + fer_bonus + custode_extra_fer
            c_pos = cella_libera_vicino(tile_id, (2, 2), celle_occupate())
            ...
            if indagine.get('chi_confermato'):
                ...
                log('    [IL NOME SBAGLIATO — D2] gli ricordate che i mandanti bruciano i '
                    'sicari come Ferri: il Sicario esita (salta la prima attivazione).')
        if tile_id == 'T4':
            log('    Nel Mercato Coperto: 1 Sgherro e 1 Sicario tra i banchi.')
            for nome_n in ('LO SGHERRO', 'IL SICARIO'):
                if pool[nome_n] <= 0:
                    pool_esauriti_totale += 1
                    continue
                pool[nome_n] -= 1
                base = NEMICO[nome_n]; a_fer = base['fer'] + fer_bonus
                cella = cella_libera_vicino(tile_id, (2, 2), celle_occupate())
                enemies.append(dict(nome=nome_n, fer=a_fer, fer_max=a_fer,
                                     dif=base['dif'], att=base['att'], dan=base['dan'] + dan_bonus,
                                     mov=base['mov'], pos=cella or porta_attuale_pos))
        if tile_id == 'T5' and (not custode or custode['fer'] <= 0):
            log('    La Salita: 2 Sgherri di retroguardia dal basso.')
            for _ in range(2):
                if pool['LO SGHERRO'] <= 0:
                    break
                pool['LO SGHERRO'] -= 1
                ...
                enemies.append(dict(nome='LO SGHERRO', fer=a_fer, fer_max=a_fer,
                                     dif=base['dif'], att=base['att'], dan=base['dan'] + dan_bonus,
                                     mov=base['mov'], pos=cella or porta_attuale_pos))
        elif tile_id == 'T5' and custode and custode['fer'] > 0:
            # il Sicario si e' portato avanti e aspetta in cima
            log('    Il Sicario Gentile si è portato AVANTI e aspetta in cima alla Salita.')
            custode['pos'] = cella_libera_vicino(tile_id, (1, 3), celle_occupate()) or porta_attuale_pos
        # Fischietto: una volta, rimuove 1 Sgherro appena piazzato
        if fischietto[0] and tile_id in ('T2', 'T4'):
            sgh = next((e for e in enemies if e['nome'] == 'LO SGHERRO' and e['fer'] > 0), None)
            if sgh:
                fischietto[0] = False
                enemies.remove(sgh)
                log('    [FISCHIETTO D’ALLARME] una ronda onesta accorre: 1 Sgherro messo in fuga.')

        fase_eroi(tappa)
        # Riva si tiene addosso alla guardia del corpo (l'eroe con piu'
        # Salute in piedi): si sposta su una cella adiacente a lui, cosi'
        # lo scudo puo' parare il primo colpo del round.
        if riva['vivo'] and vivi():
            guardia = max(vivi(), key=lambda m: salute[m])
            libere = [c for c in _vicini(pos[guardia])
                      if c not in celle_occupate() and c not in _arredi(tile_attuale)]
            riva['pos'] = libere[0] if libere else pos[guardia]
        fase_minaccia()
        fase_nemici(tappa, True)
        tick_canto()
        max_down_simultanei = max(max_down_simultanei, len(down))
        if not riva['vivo']:
            esito = 'SCONFITTA (il teste è caduto)'
            break
        if not vivi():
    ...
    if esito is None:
        if riva['vivo']:
            log('--- Riva raggiunge la barca al Molo del Lume (Interagire): VITTORIA. ---')
            esito = 'VITTORIA'
        else:
            esito = 'SCONFITTA (il teste è caduto)'
    ...
    log(f'Riva: {"salvo" if riva["vivo"] else "caduto"} (Salute {max(riva["salute"],0)}/3) | '
        f'Sicario Gentile: {"abbattuto" if custode and custode["fer"] <= 0 else ("in gioco" if custode else "mai apparso")}')
    for n in party:
...
def sessione_curva():
    """Curva completa 2-10 dell'Episodio 9, config di produzione riusata.
    Seed 690000+ (mai usati altrove)."""
    ...
    for size in (2, 3, 4, 5, 6, 7, 8, 9, 10):
        ...
        m = esegui_batch_multi_party(nome, size, 'finale_v3', 'nessuna',
                                      n_party=5, n_seed=30, seed_base=690000 + size * 1000)
        risultati.append(m)

    path = os.path.join(LOG_DIR, 'riepilogo_ep9.md')
    with open(path, 'w', encoding='utf-8') as f:
        f.write('# Episodio 9 — curva 2-10, config di produzione riusata\n\n')
    ...
    print(f'\nCurva Episodio 9 fatta. Riepilogo in {path}')
