...
def simula_spedizione(party, indagine, log, run_seed, formula_minaccia='standard',
    ...
    def fase_eroi(luogo_label):
        nonlocal chiave, tobia_libero, custode, custode_stunned, campanello_usato, canto, voce_ferma_scade_round, pannelli
        for n in vivi():
            h = HERO[n]
    ...
    if esito is None:
        ...
        while progresso < 3 and not return_fatto:
            ...
            if agganciato and not custode_stunned:
                ...
                progresso += 1
            if round_n > 60:
                esito = 'TIMEOUT (60 round: il Suggeritore non molla la presa)'
                break
        if esito is None:
            if pannelli > 0:
