...
def indagine():
    ...
    c.drawCentredString(W/2, 22*mm, 'Chi tiene il fascicolo Luoghi ordina le 8 carte per numero (è nel titolo) e le dispone in fila:')
    c.drawCentredString(W/2, 15*mm, 'le “Disponibile dall’inizio” scoperte, le altre coperte — da lì in poi la posizione vi dice il numero.')
    c.showPage()
...
def spedizione():
    ...
    frame_flow(c, 28*mm, H - 103*mm, W - 56*mm, 50*mm, [
        Paragraph(f'Le {len(MINACCE)} carte Minaccia e le {len(NEMICI)} schede Nemici sono stampate come carte a parte '
                  ...
                  'griglia, arredi e porte segnate. Le pagine seguenti sono le note per tessera, una tessera per foglio: '
    ...
    # Fronte = arte + descrizione estesa + testo letto alla rivelazione;
    # retro fisico = cosa nasconde (Cercare/carta/hook/note), solo per chi
    # arbitra - una voce per OGNI tessera, anche vuota, cosi' sfogliare non
    # rivela QUALI tessere contano. Le coppie restano consecutive nel PDF:
    # una stampa fronte/retro normale le allinea sullo stesso foglio.
    import gen_narrator as N
    for T in TILES:
        N.pagina_tessera_fronte(c, T['id'], T['nome'], N.TESSERE_DESC[T['id']],
                                N.TILE_ART[T['id']], T['testo'])
        c.showPage()
        N.pagina_retro_tessera(c, T['id'], T['nome'], T, N.oggetto_riga(T['id']))
        c.showPage()
    token_sheet(c)
    registro_ferite(c)
    c.save()
...
def token_sheet(c, groups=None):
    ...
    GROUPS = groups or [
        ...
        # Il Canto e' UN segnalino che avanza sulla sua traccia (0-8) sul
        # tabellone degli orologi: queste tre arti sono i suoi stadi, e si
        # sostituisce il pezzo al crescendo raggiunto (Il Canto Sale -> Il Coro
        # Risponde -> Il Canto Cresce), come le carte Minaccia-timer.
        ('CANTO (uno in gioco: si sostituisce al crescendo)',
         [('Il canto sale.png', 1), ('Il coro risponde.png', 1), ('Il canto cresce.png', 1)]),
    ]
