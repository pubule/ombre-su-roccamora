...
LETTERA_11 = (
    ...
    "<font name=\"OldStd-Italic\"><i>Luoghi disponibili dall’inizio: la Torre Civica, la pensione dei topografi, l’Archivio "
    "Civico e la Camera dei Pesi. Gli altri andranno sbloccati.</i></font>")
...
def indagine():
    ...
    frame_top = H - (200 - 140)*mm
    frame_y = frame_top - lett_h
    delta = (H - 200*mm) - frame_y
    frame_flow(c, mx, frame_y, avail_w, lett_h, [cap_p, let_p])
    seal(c, W - mx - 12*mm, H - 215*mm - delta, r=13*mm, angle=-10)
...
LUOGHI11_DESC = {
    1: "Sotto la Torre la piazza fa da imbuto al vento: la bora scende dai tetti, gira attorno al "
       "basamento e risale portando odore di pietra bagnata e di gas bruciato dei lampioni, che a "
       "quest’ora nessuno ha ancora spento. Il telo è tenuto agli angoli da quattro ciottoli "
       "tolti al selciato, e a ogni raffica si gonfia e si riabbassa sempre nello stesso ordine — "
       "prima i piedi, poi le spalle — come un respiro che qualcun altro conta al posto suo. Il "
       "custode arriva tenendo la lanterna bassa, le unghie nere del grasso del quadrante, e "
       "parla guardando in alto invece che a terra: «cadde dalla cella delle campane, ieri "
       "sera», dice, e ripete due volte l’ora come chi teme di essersela già dimenticata. Molto "
       "più su la cella è una bocca d’ombra fra i pinnacoli; il quadrante intanto continua a "
       "battere i quarti sopra il telo, puntuale, con quel colpo secco che nella piazza vuota "
       "arriva due volte, la seconda dai muri. Contro la grata di uno scolo, all’angolo, una "
       "pagina di taccuino sta impigliata per un lembo: è ancora asciutta.",
    2: "La pensione dei topografi sa di lana bagnata messa ad asciugare e di brodo riscaldato due "
       "volte: un odore da casa in affitto, dove nessuno cucina per restare. Nell’ingresso i "
       "treppiedi stanno contro la parete, chiusi e legati in fascio come fucili; le cordelle "
       "pendono ai chiodi arrotolate tutte nello stesso verso, e sul tavolo comune sei tazze "
       "aspettano capovolte, asciutte da ore. Si parla poco, e ogni volta che il portone di "
       "strada batte le teste si voltano insieme verso l’uscio, poi tornano giù una alla volta, "
       "l’ultima sempre con un poco di ritardo. Il caposquadra vi viene incontro con la mano "
       "tesa e le maniche già abbottonate ai polsi, cortese come chi ha preparato la frase "
       "mentre scendeva le scale: «tragico incidente, il povero Ratti», dice; «il vertigine, sa "
       "com’è, prima o poi tradisce» — e mentre lo dice sorride con la bocca soltanto. Sul "
       "pianerottolo una porta è socchiusa e la stanza dietro sta al buio: il letto è rifatto "
       "stretto, da caserma, e sul cuscino non c’è la piega di una testa.",
    3: "L’Archivio Civico ha il freddo asciutto delle stanze dove si scalda la carta e non la "
       "gente: colla d’amido, cuoio vecchio, e quel pepe che fa la polvere quando la si smuove "
       "sotto la lampada. Gli scaffali corrono fino al soffitto in file numerate a gesso, la "
       "scala a rotaia ferma a metà corridoio; i faldoni delle reti idriche stanno più in basso "
       "degli altri, consumati sugli angoli dal passaggio delle mani. L’archivista srotola le "
       "piante sul bancone e le tiene ferme con quattro pesi di piombo, uno per angolo, muovendo "
       "le dita sopra i tratti senza mai posarle sulla carta: «i vostri topografi sono venuti a "
       "copiarle tutte. Cortesi», dice, e mentre lo dice controlla la fila degli scaffali dietro "
       "di sé. Il silenzio è tale che si sente la lancetta dell’orologio a muro, e a ogni scatto, "
       "sulla pila dei tubi di latta, un cartellino trema appena — sempre lo stesso. In fondo al "
       "bancone, in un vassoio d’ottone, resta la matita rossa di un copista, temperata di fresco "
       "fino all’osso.",
    4: "Nella Camera dei Pesi l’aria è ferma e sa d’ottone lucidato, di ceralacca e di "
       "quell’aceto sottile che resta dove si strofina il metallo tutti i giorni. Sotto le "
       "campane di vetro stanno i campioni di stato in fila per grandezza, ciascuno nel suo "
       "incavo di panno verde; alla parete i regoli campione sono appesi per il foro, dal "
       "braccio al palmo, e sull’asta di legno accanto alla finestra le tavole delle maree "
       "pendono a mazzo, un foglio per giorno, quello di oggi ancora umido d’inchiostro. Il "
       "misuratore capo lavora coi guanti di filo bianco e non li toglie nemmeno per stringervi "
       "la mano; parla mentre rimette a zero il piatto della bilancia grande, e la lancetta, "
       "arrivata allo zero, continua a tremare un poco più a lungo di quanto il colpo "
       "giustifichi: «i vostri topografi venivano a tarare le cordelle qui», dice, senza alzare "
       "gli occhi dal quadrante. Sul banco, dove la polvere è intatta, restano i segni di un "
       "treppiede: tre piccoli tondi, e il terzo un poco più profondo degli altri.",
    5: "Lo studio sta in una via qualunque, e la prima cosa che si sente, aperta la porta, è che "
       "dentro fa più freddo che fuori: nessuno ci ha acceso una stufa quest’inverno. La luna "
       "entra a lame dalle stecche delle persiane e attraversa una stanza sola — uno scrittoio "
       "sgombro, una lampada col vetro pulito e il serbatoio asciutto, un timbro appoggiato sul "
       "suo cuscinetto — e più niente: non un registro, non una sedia per chi aspetta, non un "
       "chiodo alla parete dove sia stato appeso qualcosa. Dietro l’uscio la posta si è "
       "accumulata da sé in un ventaglio di buste da ufficio, nessuna aperta; nel filo d’aria "
       "che passa sotto la porta quella in cima si solleva di un lembo e si riposa, sempre allo "
       "stesso punto, con un fruscio da foglia secca. Sul piano dello scrittoio la polvere è "
       "pari dappertutto tranne che in un rettangolo netto, grande come un registro. La targa "
       "fuori è d’ottone nuovo, con una sigla sola incisa, e il muro attorno è stato ridipinto "
       "di un bianco che non è quello della facciata.",
    6: "Ottanta gradini di pietra e poi il legno, e a ogni giro il freddo cambia odore: prima "
       "muffa e cera, poi corda bagnata, poi quel sapore di moneta che dà il bronzo quando è "
       "molto grande e molto vicino. Nella cella il vento entra dalle feritoie da tutti i lati "
       "insieme e non decide da dove: fa girare la polvere in tondo sulle assi, e le corde che "
       "scendono nel pozzo delle scale oscillano tutte di un dito, ferme le campane. Il "
       "campanaro sente da un orecchio solo e vi tiene la lanterna vicino alla faccia per "
       "leggervi la bocca; ha le mani gonfie dei nodi di chi ha tirato canapa per cinquant’anni. "
       "«i vostri amici topografi ci saltano come gatti, di notte», dice, e ride di due note. "
       "Sul leggio il registro dell’accordatura è aperto, le colonne scritte a matita e non a "
       "inchiostro; oltre le feritoie i tetti stanno sotto come una carta stesa, i canali neri, "
       "le altre torri che spuntano dal buio a una a una. Sul davanzale di una feritoia qualcuno "
       "ha lasciato un gessetto rosso, consumato da una parte sola.",
    7: "Il sagrato di notte è pietra bagnata senza che abbia piovuto: l’umido del canale sale "
       "dalle commettiture e la luna ci si posa sopra come una velatura d’olio, tanto che si "
       "cammina piano per non scivolare. La facciata è un muro di buio con dentro nicchie più "
       "buie ancora; il portone è chiuso, e dalla fessura sotto le ante esce un filo d’aria "
       "tiepida che sa d’incenso freddo e di cera spenta — l’aria di una stanza grandissima che "
       "respira dalla porta. Il sagrestano vi viene incontro col mazzo di chiavi stretto in "
       "pugno perché non suoni, la tonaca tirata su con una mano, e resta sempre a due passi, "
       "mai di fronte: «rilievi, qui? Nessuno mi ha avvisato», dice, e intanto guarda i vostri "
       "piedi. Nel silenzio, a intervalli lunghi, dal fondo della piazza torna il rumore "
       "dell’acqua contro i gradini, e ogni volta un poco più tardi di quanto l’ora giustifichi. "
       "A metà del sagrato, fra le lastre grigie consumate a conca dai passi, una lastra è più "
       "nuova delle altre: liscia, senza iscrizione, e più asciutta.",
    8: "La bottega del cordaio sa di canapa nuova e di sego caldo, un odore grasso che si attacca "
       "in fondo alla gola, e ci fa più caldo di quanto una stufa spenta possa spiegare. Le "
       "matasse pendono ai ganci del soffitto in ordine di grossezza, dalla cordicella da "
       "campanello alla gomena grossa come un polso; la stanza è lunga il doppio di quanto la "
       "facciata prometta, e in fondo, al buio, sta il filatoio con la ruota ferma e il carrello "
       "arrivato in cima alla corsa. Il cordaio lavora seduto, la manica destra logora fino alla "
       "trama, e passa la mano lungo il trefolo come si passa sul collo di una bestia, sempre "
       "nello stesso verso: «le mie funi tengono un uomo meglio di un bronzo», dice, e non alza "
       "gli occhi dal lavoro. Le matasse appese girano lentissime sui ganci, tutte insieme, "
       "prima in un verso e poi nell’altro, senza che nessuno le tocchi. Sul banco, in un piatto "
       "di stagno, sta un capo di fune tagliato di netto, ancora legato con lo spago del suo "
       "cartellino.",
    9: "Il ponteggio fascia il fianco della Torre per tutta l’altezza, e di notte è una cosa che "
       "parla: le tavole scricchiolano per conto loro, le corde di legatura fischiano sottili "
       "dove il vento le prende di taglio, e ogni tanto un secchio appeso più in alto batte "
       "contro un montante, sempre due colpi. Sa di legno bagnato, di calce spenta e di quel "
       "ferro dolciastro che hanno le impalcature dopo giorni di pioggia. Alla base, oltre il "
       "cavalletto che sbarra il passo, i primi pioli sono lucidi nel mezzo e le tavole del "
       "primo ripiano sono state raschiate dalle suole fino al legno chiaro; le altre no. Il "
       "guardiano notturno tiene le mani nelle maniche e non si stacca dal braciere: «col vento "
       "di stanotte, lassù, non ci salirei per tutto l’oro del mondo», dice, e alza gli occhi un "
       "istante prima di tornare al fuoco. Sopra di lui i tetti si perdono nel nero a scaglioni, "
       "uno più basso dell’altro. A un chiodo del primo montante è appesa una lanterna, spenta, "
       "col vetro ancora tiepido.",
}
...
TESSERE_DESC_11 = {
    'T1': "Nell’abbaino il freddo entra prima della luce: dalle fessure delle assi il vento "
          "fischia in due note diverse, una più bassa dell’altra, e il cero piantato nella "
          "propria cera piega la fiamma verso l’interno invece che verso l’apertura. Sa di guano "
          "di piccione, di pioggia vecchia dentro il legno, di calce smossa da poco. Contro la "
          "parete stanno le casse del restauro, marchiate a fuoco e impilate a due a due, e "
          "sopra la pila un rotolo di rete e un paio di guanti da muratore lasciati aperti come "
          "li ha tolti una mano. La botola sul tetto è già alzata, e il quadrato di cielo che "
          "ritaglia è più chiaro della stanza: ci passa dentro un’aria che sa d’acqua e di "
          "ghiaccio, l’odore che le cose hanno a venti braccia da terra. Dal buco sale la città "
          "come un suono solo, basso, fatto di tutto insieme, e dentro quel suono, ogni tanto, "
          "il ferro di una banderuola che gira, si ferma di schianto e riparte. Nella polvere di "
          "calce, davanti alla scaletta, resta una sola impronta di suola, con la punta rivolta "
          "alla botola.",
    'T2': "Fuori dall’abbaino il vento non soffia: spinge, e spinge di traverso, a colpi che "
          "arrivano da ponente e finiscono di netto, lasciando ogni volta un mezzo secondo di "
          "silenzio in cui si sente il proprio sangue. Il camminamento è largo un piede e mezzo, "
          "pietra levigata dall’acqua di secoli, e non ha ringhiera: da un lato il fianco della "
          "Torre, freddo attraverso la manica, dall’altro il vuoto e la piazza in fondo, piccola "
          "come un francobollo, coi lampioni ridotti a punte di spillo e il rumore di laggiù "
          "che arriva su in ritardo, come dal fondo di un pozzo. Quassù non c’è polvere — "
          "il vento ha portato via tutto ciò che non è inchiodato — e la pietra è nuda e lucida "
          "come un osso. Il gelo ha lasciato sulle tegole del filare basso quel velo che viene "
          "prima del ghiaccio e che sotto la suola non fa rumore. A metà passaggio una tegola, "
          "sotto la suola, si muove: non cade, si sposta di un dito e torna, e continua a "
          "muoversi anche dopo che il piede è passato oltre.",
    'T3': "Fra i bronzi il vento smette come dietro una porta chiusa, e il cambio è così netto "
          "che le orecchie fischiano nel silenzio: qui l’aria è ferma, sa di guano e di grasso "
          "da perni, e la pietra rimanda addosso il fiato. Le campane pendono dai ceppi di "
          "rovere, enormi da vicino quanto non sembrano dal basso, e ognuna tiene sotto di sé la "
          "propria macchia d’ombra, più nera del resto del pavimento. I battagli non battono, ma "
          "le folate che passano di fuori li fanno sfiorare il metallo, e dal bronzo esce un "
          "ronzio basso che non si sente con le orecchie: si sente nello sterno e nei denti, e "
          "resta un poco più a lungo di quanto la folata duri. Nella nicchia dove il campanaro "
          "tiene la sua roba stanno un fiasco impagliato, uno scaldino di rame e una matassa di "
          "corda di servizio arrotolata a otto. Sul ceppo della campana di mezzo qualcuno ha "
          "inciso una data e poi l’ha raschiata via, lasciando il legno più chiaro nella forma "
          "dei numeri.",
    'T4': "Il tetto sale a due falde e il colmo è una lama di coppi larga quanto un piede, con la "
          "calcina delle giunture sbriciolata che scorre sotto la suola come sabbia. Qui il "
          "vento non ha un verso: gira, spinge da destra per tre respiri e poi da sinistra senza "
          "avvisare, e ciò che vi teneva su un momento prima è la stessa cosa che vi butta giù. "
          "Alla gronda la lamiera è marcia di ruggine fino alla trasparenza e sotto il peso "
          "della propria acqua si è staccata dai chiodi per un buon braccio, e pende nel vuoto "
          "battendo contro il muro a ogni raffica, sempre con la stessa nota. Sul filo della "
          "falda, in piedi come se il ripido non lo riguardasse, un uomo in redingote grigia "
          "sbarra il passaggio col treppiede alzato di traverso: nocche bianche sul legno, "
          "colletto ancora abbottonato, la faccia di chi ha ricevuto un ordine e non se l’è "
          "fatto spiegare. Dietro di lui, incastrato fra due coppi, sta un secondo treppiede, "
          "chiuso, con la sacca ancora allacciata.",
    'T5': "Il ballatoio gira attorno alla Torre in un anello di pietra largo due passi, appena "
          "sotto la cella, e camminarci è come stare fermi su una cosa che si muove: a ogni "
          "spigolo la città ruota sotto i piedi — tetti, canali, il porto in fondo ridotto a due "
          "lumini — e torna uguale un momento prima di quando ve l’aspettate. La balaustra "
          "arriva al ginocchio, consumata a onde dal vento e levigata come una saponetta; non "
          "c’è appiglio e non c’è riparo, e quassù il vento non fa più raffiche: è una mano "
          "sola, aperta, appoggiata contro il fianco. Da ponente la bora porta lo spruzzo del "
          "mare, e il sale si sente sulle labbra a un miglio dall’acqua; il freddo arriva tutto "
          "da quel lato solo, e una guancia resta calda mentre l’altra non sente più. Oltre lo "
          "spigolo, più "
          "in alto, un uomo passa contro il cielo con un rotolo di carta stretto al petto e non "
          "guarda giù nemmeno una volta. Alla base del muro, impigliato in una sbavatura di "
          "malta, resta un lembo di stoffa grigia: si muove ancora.",
    'T6': "La guglia è l’ultimo dito di pietra della Torre, e quassù il vento non spinge più: "
          "urla, e nell’urlo le parole si perdono a un palmo dalla bocca — ci si intende a "
          "gesti, o non ci si intende. Il ghiaccio ha velato di un vetro sottile tutto il lato "
          "di ponente, e la luna lo fa grigio; a levante, dove la bora non batte, il granito è "
          "asciutto e ruvido, e la differenza fra i due lati le dita la sentono prima degli "
          "occhi. Non c’è più su: sopra la palla di rame resta solo la banderuola, che gira, si "
          "ferma di schianto contro il vento e riparte nell’altro verso. Ivo Speranza sta "
          "aggrappato con una mano al cornicione e con l’altra tiene un rotolo di carta dentro "
          "la giacca; ha il fiato corto, gli occhi che misurano la distanza fra sé e voi, e "
          "mezza suola già oltre il bordo. Sotto di lui la città è un’altra cosa: buia, piccola, "
          "piena di campane ferme. Dove il ghiaccio si è sciolto sotto una mano, sulla pietra "
          "resta il segno di cinque dita.",
}
