...
4 luoghi + 2 oggetti; stesse regole delle altre sezioni (stesso stile, stesso
...
```
full bleed dark fantasy painting, dim private club salon in an 1889 canal palazzo, six empty leather armchairs around a round table, five old portraits on the wall and one empty hook, beeswax candles, dust sheets half removed, quiet and solemn, 1889 gaslamp gothic, oil painting, muted teal and crimson palette with gold accents --ar 3:4 --style raw --no frame, border, card, text, letters, watermark
```

**P2 — La Taverna della Chiatta** → `artworks/Taverna della Chiatta.png`
```
full bleed dark fantasy painting, small canal-side tavern interior at dusk seen from behind the counter, an innkeeper polishing a cloudy glass, through the window a bridge and the lit portal of a palazzo across the canal, oil lamps, worn wooden tables, 1889 gaslamp gothic, oil painting, muted teal and crimson palette with gold accents --ar 3:4 --style raw --no frame, border, card, text, letters, watermark
```

**P3 — Il Banco dei Pegni di Fossa** → `artworks/Banco dei Pegni.png`
```
full bleed dark fantasy painting, cramped 19th century pawnbroker shop behind an iron grate, shelves of pledged silverware pocket watches and violins, an old pawnbroker examining a ring through a loupe by lamplight, thick ledger open on the counter, 1889 gaslamp gothic, oil painting, muted teal and crimson palette with gold accents --ar 3:4 --style raw --no frame, border, card, text, letters, watermark
```

**P4 — La Dogana Vecchia** → `artworks/Dogana Vecchia.png`
```
full bleed dark fantasy painting, derelict 19th century customs house rotting on a canal at dusk, collapsed jetty, bricked-up gates, a lone barge moored where nothing should be, a man pretending to fish from the pier, black still water, 1889 gaslamp gothic, oil painting, muted teal and crimson palette with gold accents --ar 3:4 --style raw --no frame, border, card, text, letters, watermark
```

**L'Anello di Chiavi** → `artworks/Anello di Chiavi.png`
```
...
**Tessere mappa** (sostituisci AMBIENTE) — **v3**: v1 e v2 (sotto, tenute
per memoria) chiedevano una mappa disegnata a inchiostro su pergamena, ed
entrambe sono uscite in isometria/prospettiva invece che in pianta vera.
L'utente ha condiviso un riferimento visivo concreto: **battlemap dipinta
stile VTT** (Foundry/Roll20), non una mappa a china — vista dall'alto
ortogonale vera, pietra e muri dipinti con texture (non linee di contorno),
luce calda di torce contro ombre blu/teal fredde, muri come blocchi piatti
spessi, niente griglia incisa nell'arte (la griglia 4x4 la disegna già
`generate-tiles.js` sopra). Riferirsi a questo stile esplicitamente
("painted top-down tabletop RPG battle map") invece che a "mappa disegnata
a mano" sembra la chiave per uscire dall'isometria, sulla base del
riferimento fornito:
```
painted top-down tabletop RPG battle map tile, straight overhead orthographic camera, flat 2D game map asset, weathered stone brick floor and thick stone walls with cracks and moss, AMBIENTE, warm flickering torchlight pools against cool blue-teal ambient shadow, moody dark fantasy digital painting, no ink linework, no parchment --ar 1:1 --style raw --no isometric, 3D render, perspective, angled view, side view, ink, pencil sketch, parchment, sepia, grid lines, text, letters, words, watermark
```
Se esce ancora in isometria, il colpevole probabile è di nuovo `--style raw`
(spinge verso un render pittorico più "scenico"/prospettico): togli
`--style raw` e riprova, oppure aggiungi `--no shadow depth, extruded walls`.
