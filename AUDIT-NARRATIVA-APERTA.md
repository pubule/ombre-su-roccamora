...
**stato: decisa** — decisa (09/08, scelta dell'autore): **la banda 55-75% resta il riferimento, ma non e' piu' il giudice.** Il pilota filtra — dice dove guardare, e lo dice bene — ma non dice se l'ansia c'e', se il tavolo ha capito la scena, se la serata regge: tre dei quattro KPI del progetto non hanno un numero. Le decisioni di taratura si prendono su **partite vere**.

Non e' una posizione di principio, e' quello che ha detto la sessione: su **tre** episodi aperti sospettando uno sbilanciamento — Ep. 10, Ep. 11, Ep. 20 — **tre volte** il motore stava giocando qualcosa di diverso da quello stampato, e nessuno dei tre era un problema di numeri. Col pilota come giudice avrei tarato tre episodi sani per compensare tre difetti.

Ne segue una regola operativa, scritta in `BILANCIAMENTO.md`: **prima di toccare un numero, verificare che il motore stia giocando l'episodio del fascicolo.**

Il resto — quali episodi portare al tavolo per primi e cosa riportare indietro da ogni sessione — e' nella coda di `BILANCIAMENTO.md`. La percentuale del finale resta dov'e' finche' qualcuno non lo gioca: vedi [[N-02]]. · riferimenti: src/gen_ep20.py (ritmo e pressione), scripts/simulate_ep20.py, webapp/data/ep20.json
...
**Resta il difetto di fondo, ed e' lo stesso dell'Ep. 20:** il budget lo mangia il tragitto. Si arriva alla camera al round 9-10 e la Demolizione chiude al 12, quindi la seconda via si puo' percorrere solo se si abbatte un boss da 5 Ferite in due round. Il numero misurato resta a una cifra. Non l'ho tarato: e' la stessa domanda aperta di [[N-110]] — prima di alzare un numero, decidere se la banda 55-75% valga ancora. · riferimenti: webapp/data/ep10.json (compiti.prova quante=14, orologio demolizione max=12 ogni=1)
