// Misura l'Ep.1 giocandolo davvero in modalità digitale (Playwright).
//
...
//
// Uso: node webapp/server.js ; node webapp/misura-ep1.mjs [partite]
...
function scoreArredi(c, tentati) {
  ...
  if (c.t !== USCITA_TILE) { const d = dirVerso(c.t, USCITA_TILE); return 100 + (d ? verso(d, c.x, c.y) : 0); }
...
const P = () => pg.evaluate(() => JSON.parse(localStorage.getItem('osr.partita.ep1')));
...
async function meta() {
  ...
  const k = (p.indagine.oggetti || []).some((o) => /chiave della cella/i.test(o));
  return (!k && !s.cercate?.T4) ? 'T4' : 'T6';
}
...
async function turnoEroe(nm, mt) {
  ...
  for (let k = 0; k < 5; k++) {
    ...
    else if (lib('cercare') && pos.t === 'T4' && !s.cercate?.T4 && await vis('#az-cercare')) { tipo = 'cercare'; await clicDom('#az-cercare'); await sciogli(); }
...
for (let g = 0; g < N; g++) {
  ...
  await pg.evaluate((p) => {
    ...
    localStorage.setItem('osr.partita.ep1', JSON.stringify({
      v: 1, episodio: 'ep1', modo: 'digitale', party: p, creata: Date.now(), fase: 'spedizione',
  ...
  for (let r = 0; r < 80; r++) {
    ...
    if (!allIngresso && (s.rivelate || []).includes('T6')) {
      allIngresso = {
        ...
        inT6: (s.nemici || []).filter((n) => n.pos && n.pos.t === 'T6').length,
  ...
  console.log(`${String(g + 1).padStart(2)}/${N}  ${String(f.esito || 'stallo').padEnd(10)} round ${String(f.round).padStart(2)}` +
    `  T6 ${allIngresso ? `r${allIngresso.round} eroi ${allIngresso.vivi} salute ${allIngresso.salute} nemici ${allIngresso.nemici}(${allIngresso.inT6} in T6) canto ${allIngresso.canto}` : 'mai'}` +
...
if (ing.length) {
  ...
  console.log(`\n--- all'ingresso in T6 (la stanza del boss), ${ing.length} partite ---`);
