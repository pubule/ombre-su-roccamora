...
import { rendi, norm, costruisciMazzo, carteDaPescare, pesca, fineRound,
         cantoDaCarta, cerca, urlArt, cartaOggetto, tettoCanto,
         sogliaCanto, cartaGrandeHtml } from './engine.js';
  ...
  const html = `<div class="barra"><span></span><div class="titolo">${esc(aperta.titolo || 'minaccia')}</div><span></span></div>
    <div class="pannello">
      ${aperta.carta
        ...
        ? `${cartaGrandeHtml(aperta.carta, 'minaccia')}
...
function messaggioCarta(titolo, carta, annunci) {
  return new Promise((ok) => {
    ...
    app.innerHTML = `<div class="barra"><span></span><div class="titolo">${esc(titolo)}</div><span></span></div>
      <div class="pannello">
        ${cartaGrandeHtml(carta, 'minaccia')}
...
function aggancia() {
  ...
  app.querySelectorAll('[data-obj]').forEach((b) => b.onclick = () => {
    ...
    messaggio(nm.toLowerCase(), `${cartaGrandeHtml(c, 'oggetto')}
  ...
  app.innerHTML = `<div class="barra"><button class="btn" id="nav-esci">← menu</button>
    ...
    <div class="pannello centrato">
      ...
      <p class="mt">${sp.esito === 'vittoria'
        ? `${specScortati().map((s) => s.nome).join(' e ') || 'Il gruppo'} è al sicuro. ${sp.round} round, canto ${sp.canto}.`
        : sp.esito === 'parziale'
